...
def load_checkpoint(checkpoint_weights_path, checkpoint_config_path):
    ...
    example_batch = {
        ...
        "goals": {"image": np.zeros((1, FLAGS.im_size, FLAGS.im_size, 3), dtype=np.uint8)},
...
def main(_):
    ...
    while True:
        ...
        if ch == "y":
            ...
            obs = env.current_obs()
            image_goal = (obs["image"].reshape(3, FLAGS.im_size, FLAGS.im_size).transpose(1, 2, 0) * 255).astype(
                np.uint8
            )
